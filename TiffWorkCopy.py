...
def save_dem_as_aligned_obj_points(dem_data, output_path):
    """
    Save the original DEM as a point cloud OBJ file,
    aligned with mesh and edge OBJ outputs (X, Z, -Y and shifted Z).
    """
    h, w = dem_data.shape
    verts = []
    ...
    for y in range(h):
        for x in range(w):
            z = dem_data[y, x] / 30  # ❶ Normalize Z
            verts.append((x, -y, z))  # ❷ Flip Y, keep Z positive

    min_z = min(v[2] for v in verts)  # ❸ Ground shift to Z = 0

    with open(output_path, "w") as f:
        f.write("# DEM points aligned with triangulated mesh\n")
        for x, y_neg, z in verts:
            f.write(f"v {x} {z - min_z} {-y_neg}\n")  # X, (height), Y
...
if __name__ == '__main__':
    ...
    # data = read_tiff("C:/Users/ariks/uni/ResearchStudent/SAR_Imageas_02_01/Picture1_full/output_hh.tif")
    # save_dem_as_aligned_obj_points(data, "C:/Users/ariks/uni/ResearchStudent/SAR_Imageas_02_01/Picture1_full/dem_points.obj")
    ...
    points =np.array([v[:2] for v in load_obj_vertices_only("C:/Users/ariks/uni/ResearchStudent/SAR_Imageas_02_01/Picture1_full/dem_points.obj")])
    ...
    def get_constraints_from_edge_obj(edge_obj_path, target_points):
        """
        Load edge lines from OBJ and match to indices in target_points using KDTree.
        Returns list of (index1, index2) constraints.
        """
        edge_vertices, edge_lines = load_edge_obj(edge_obj_path)
        tree = KDTree(target_points)
        edge_vertices = [(lst[0], lst[-1]) for lst in edge_vertices if lst]
        matched_indices = []
        for v in edge_vertices:
            idx = tree.query((v[0], v[1]))[1]
            matched_indices.append(idx)

        constraints = []
        for i1, i2 in edge_lines:
            a = matched_indices[i1]
            b = matched_indices[i2]
            if a != b:
    ...
    def print_range_stats(label, point_array):
        x_vals, y_vals = point_array[:, 0], point_array[:, 1]
        print(
            f"{label} - X: ({x_vals.min():.2f}, {x_vals.max():.2f}) | Y: ({y_vals.min():.2f}, {y_vals.max():.2f}) | Count: {len(point_array)}")


    def print_index_stats(label, edges):
        all_indices = [i for a, b in edges for i in (a, b)]
        print(f"{label} - Index Range: ({min(all_indices)}, {max(all_indices)}) | Total edges: {len(edges)}")


    print("=" * 40)
    print_range_stats("DEM Points", points)
    print_range_stats("Triangulated Points", t['vertices'])

    print_index_stats("Delaunay Edges", edges_delaunay)
    print_index_stats("Internal Constraints", internal_constraints)
    print_index_stats("Constrained Triangulation Edges", edges_constrained)
    print("=" * 40)
    ...
    def save_triangulated_points_with_height_in_middle(tri_points, original_points, save_path):
        from scipy.spatial import cKDTree

        # Match each triangulated (x, y) to closest original (x, y)
        x_y_points = [(lst[0], lst[-1]) for lst in original_points if lst.size > 0]

        tree = cKDTree(x_y_points)
        _, indices = tree.query(tri_points)

        # Take Z (height) from original
        z_vals = original_points[indices, 1]

        # Build final 3D points: v x height y
        obj_lines = [
            f"v {x:.6f} {z:.6f} {y:.6f}\n"
            for (x, y), z in zip(tri_points, z_vals)
        ]
        ...
        print(f"✅ Saved {len(obj_lines)} vertices to: {save_path}")


    # Given:
    # - t['vertices'] ← 2D triangulated points
    # - t['triangles'] ← triangle faces (indices)
    # - points ← original 3D DEM points
    # save_triangulated_points_with_height_in_middle(
    #     tri_points=t['vertices'],  # 2D triangulated
    #     original_points=np.array(load_obj_vertices_only("C:/Users/ariks/uni/ResearchStudent/SAR_Imageas_02_01/Picture1_full/dem_points.obj")),  # MUST include Z
    #     save_path="triangulated_height_in_middle.obj"
    # )
    ...
    triangulated_points = t['vertices']
    plot_edges(axes[1], triangulated_points, edges_constrained, "Constrained Delaunay", constraint_edges=internal_constraints)
